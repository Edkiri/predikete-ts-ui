...
export const useProfilePic = (picUrl?: string | null) => {
  const [profilePicUrl, setProfilePicUrl] = useState(DEFAULT_IMG);

  // useEffect(() => {
  //   if (user?.profile.picture) {
  //     axios
  //       .get(`${API_URL}/users/profile-image/${user.profile.picture}`, {
  //         responseType: 'blob',
  //       })
  //       .then((res) => {
